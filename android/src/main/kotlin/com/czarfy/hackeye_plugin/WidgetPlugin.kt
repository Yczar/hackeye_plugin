...
class WidgetPlugin: FlutterPlugin, MethodChannel.MethodCallHandler, ActivityAware,
    ...
    companion object {
        private const val PREFERENCES = "WidgetPreferences"

        private const val INTERNAL_PREFERENCES = "InternalWidgetPreferences"
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        channel = MethodChannel(binding.binaryMessenger, "hackeye_widget")
        channel.setMethodCallHandler(this)

        eventChannel = EventChannel(binding.binaryMessenger, "hackeye_widget/updates")
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "initiallyLaunchedFromHomeWidget" -> {
                return if (activity?.intent?.action?.equals(WidgetLaunchIntent.WIDGET_LAUNCH_ACTION) == true) {
    ...
    private fun createReceiver(events: EventChannel.EventSink?): BroadcastReceiver {
        return object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                if (intent?.action.equals(WidgetLaunchIntent.WIDGET_LAUNCH_ACTION)) {
